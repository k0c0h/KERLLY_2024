...
void dibujarTablero(sf::RenderWindow& ventana, int** tablero, int n, const sf::Texture& reinaTextura, const vector<sf::Vector2i>& posicionesReinas, vector<sf::Vector2i>& recorridoReina) {
    ventana.clear();
    ...
    // Dibujar el recorrido de la reina seleccionada
    if (!recorridoReina.empty()) {
        for (auto& casilla : recorridoReina) {
            sf::RectangleShape recorrido(sf::Vector2f(80, 80));
            recorrido.setPosition(casilla.x * 80, casilla.y * 80);
            recorrido.setFillColor(sf::Color(0, 255, 0, 100)); // Color verde translúcido para el recorrido
            ventana.draw(recorrido);
        }
    }
    ...
    for (int columna = 0; columna < n; columna++) {
        ...
        if (esSeguro(tablero, fila, columna, n)) {
            *(*(tablero + fila) + columna) = 1; 
            posicionesReinas.push_back(sf::Vector2i(columna, fila)); // Guardar la posición de la reina
            ...
            *(*(tablero + fila) + columna) = 0; 
            posicionesReinas.pop_back(); // Eliminar la posición de la reina si no es válida
            dibujarTablero(ventana, tablero, n, reinaTextura, posicionesReinas, recorridoReinaVacío);
        }
    }
    return false;
...
int main() {
    ...
    vector<sf::Vector2i> recorridoReina;   // Mantener el recorrido de la reina seleccionada
    ...
    bool mostrarRecorrido = false; // Variable para controlar si se debe mostrar el recorrido
    ...
    while (ventana.isOpen()) {
        ...
        while (ventana.pollEvent(evento)) {
            ...
            if (evento.type == sf::Event::MouseButtonPressed) {
                ...
                // Verificar si se hizo clic en una reina
                for (auto& posicion : posicionesReinas) {
                    if (posicion.x == x && posicion.y == y) {
                        // Mostrar el recorrido de la reina
                        recorridoReina.clear();
                        // Aquí calculamos el recorrido posible de la reina, por ejemplo, en diagonal
                        for (int i = 0; i < tamanio; i++) {
                            if (i != x) recorridoReina.push_back(sf::Vector2i(i, y)); // Movimiento horizontal
                            if (i != y) recorridoReina.push_back(sf::Vector2i(x, i)); // Movimiento vertical
                        }
                        break;
                    }
                }
        ...
        // Dibujar el tablero y las reinas, mostrando el recorrido si corresponde
        dibujarTablero(ventana, tablero, tamanio, reinaTextura, posicionesReinas, recorridoReina);
    }
