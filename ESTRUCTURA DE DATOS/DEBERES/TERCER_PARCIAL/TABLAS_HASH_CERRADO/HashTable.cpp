...
int HashTable<T>::hashFunction2(const T& key) {
    ...
    if constexpr (std::is_same<T, std::string>::value) {
        // Calcular un hash robusto para cadenas
        for (char c : key) {
            keyValue = ((keyValue * 31) + c) ; // 31 es un número primo común en funciones de hash
        }
    } else {
        keyValue = key;  // Para enteros, se usa directamente
    }
    int secondHash = 7 - (keyValue % 7);
    std::cout << "keyValue = " << keyValue << ", secondHash = " << secondHash << std::endl;  // Depuración
    // **Pausa para ver la operación**
    std::cout << "Presiona ENTER para continuar...\n";
    std::cin.get();
    return secondHash;  // Fórmula de segundo hash (igual que el simulador)
}
...
void HashTable<T>::insert(T key, int method) {
    ...
    while (!inserted) {
        ...
        std::cout << "i = " << i << ", probeIndex = " << probeIndex << std::endl;  // Depuración
        // **Pausa para ver la operación**
        std::cout << "Presiona ENTER para continuar...\n";
        std::cin.get();
        ...
        } else if (table[probeIndex].value() == key) {  
            i++;  // Si la celda está ocupada con el mismo valor, continuamos
        } else {  
            i++;  // Si la celda está ocupada con otro valor, continuamos
        }
    }
