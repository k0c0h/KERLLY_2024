...
template <typename T>
bool Validaciones<T>::validarCedula(int cedula) {
    string cedulaStr = to_string(cedula);

    if (cedulaStr.length() != 10) return false;

    int primerDigito = cedulaStr[0] - '0';
    if (primerDigito < 1 || primerDigito > 24) return false;

    return true;
}

template <typename T>
bool Validaciones<T>::validarTelefono(int telefono) {
    string telefonoStr = to_string(telefono);
    if (telefonoStr.length() != 9) return false;
    return true;
}
