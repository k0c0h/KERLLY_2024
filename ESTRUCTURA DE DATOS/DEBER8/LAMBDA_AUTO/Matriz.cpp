...
T Matriz<T>::sumaRecursivaMatrices(const Matriz<T>& m1, const Matriz<T>& m2, int f, int c) {
    auto sumarMatrices = [&m1, &m2](auto&& self, int fila, int col) -> T {
        if (fila < 0) return 0;
        if (col < 0) return self(self, fila - 1, m1.columnas - 1); 
        return m1.getElemento(fila, col) + m2.getElemento(fila, col) + self(self, fila, col - 1);
    };

    return sumarMatrices(sumarMatrices, f, c);
}
